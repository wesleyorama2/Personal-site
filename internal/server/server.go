...
func (s *Configuration) makeHTTPServer() *http.Server {
	...
	rootSite := http.FileServer(neuteredFileSystem{http.Dir(s.FilesRoot)})
	vendorSite := http.FileServer(neuteredFileSystem{http.Dir(fmt.Sprintf("%s/vendor", s.FilesRoot))})
	vendorCSS := http.FileServer(neuteredFileSystem{http.Dir(fmt.Sprintf("%s/css", s.FilesRoot))})
	vendorSiteBootstrapJS := http.FileServer(neuteredFileSystem{http.Dir(fmt.Sprintf("%s/vendor/bootstrap/js", s.FilesRoot))})
	vendorJquery := http.FileServer(neuteredFileSystem{http.Dir(fmt.Sprintf("%s/vendor/jquery", s.FilesRoot))})
	vendorJqueryEasing := http.FileServer(neuteredFileSystem{http.Dir(fmt.Sprintf("%s/vendor/jquery-easing", s.FilesRoot))})
	vendorJS := http.FileServer(neuteredFileSystem{http.Dir(fmt.Sprintf("%s/js", s.FilesRoot))})
